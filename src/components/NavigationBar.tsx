  ...
  return (
    <header className="w-full bg-[#0B192C] text-white font-sans border-b border-slate-800 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!minimal && (
          <nav className="flex items-center gap-1 overflow-x-auto pb-3">
            {navItems.map((item) => (
              <Link
                key={item.to}
                to={item.to}
                activeOptions={{ exact: item.to === "/" }}
                className="whitespace-nowrap rounded-md px-3 py-2 text-sm font-semibold text-slate-200 transition-colors hover:bg-slate-800 data-[status=active]:bg-emerald-500 data-[status=active]:text-[#0B192C]"
              >
                {item.label}
              </Link>
            ))}
